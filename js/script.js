let weatherAPIKey = "c1d7d33d7d24cecf22e10bc1f3ac2148";
...
let weatherCity = "";
let autoLoc = "https://api.openweathermap.org/geo/1.0/reverse?limit=5&appid=" + weatherAPIKey;
...
navigator.geolocation.getCurrentPosition(async(pos) => {
    ...
    let endpoint = autoLoc + "&lat=" + lat + "&lon=" + lng;
    let response = await fetch(endpoint);
    let result = await response.json();
    //console.log(result);
    Swal.fire(result[0].name, "City", "success");
    weatherCity = result[0].name;

    let weather = await getWeatherByCityName(weatherCity);
    //console.log(weather);
    ...
    Swal.fire("OOps...", "Location Don't detect", "error");
}, options);
...
let weatherForCity = async(city) => {
    ...
    //console.log(weather);
    if (weather.cod === '404') {
        Swal.fire("Opps....", " You type wrong City Name.", "error");
        return;
...
searchInp.addEventListener("input", async() => {
    ...
    result.forEach((city) => {
        ...
        options.value = `${city.name},${city.state?","+city.state:" "},${city.country}`;
